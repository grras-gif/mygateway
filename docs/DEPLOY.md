...
项目需要绑定一个 KV 命名空间，并把它暴露为环境绑定 `DB`。MyGateway 的所有持久化
配置、用量聚合和可选请求日志都保存在该命名空间内。
...
- KV 是 schema-less 的，因此不再有 SQL migration 步骤。基础设置和模型价格由
...
`X-Gateway-Timing` 查看缓存命中、KV 查询、上游首包和网关首包耗时；标准 `Server-Timing`
...
- KV 没有 schema 迁移，新版本必须能直接读取旧数据；新增字段请保持向后兼容的读取默认值
...
EdgeOne Makers 支持回滚到历史部署版本，静态资源与边缘函数会一起回退。KV 中的数据不会
...
3. **KV 未绑定**：未把 KV 命名空间绑定为 `DB` 时，所有后端接口都会因缺少数据绑定而失败。
...
6. **误以为需要 migration**：KV 无 schema，无需执行任何 SQL；若控制台看不到基础设置/价格，检查 KV 绑定与 seed 日志事件。
...
部署后至少确认：项目构建成功、管理控制台可打开、KV 绑定生效，并对健康页或一个已配置的
...
| KV 读取 | 按命名空间配额计费 | 索引键让鉴权与路由保持单次 `get`；短 TTL isolate 缓存降低读放大 |
| KV 写入 | 按命名空间配额计费 | 每个完成请求写入 Analytics、密钥用量和可选日志 |
| KV 存储 | 按命名空间配额计费 | 分钟聚合 30 天、请求日志 1–7 天（控制台可调）、密钥用量 30 天 |
| 日志 | 平台采样保留 | 抽样与脱敏事件 |

降低额度的关键做法：

- 热请求使用有容量上限的 isolate 内存缓存，缓存命中时不访问 KV；冷请求通过少量 `get`
  完成鉴权与路由，唯一性索引（`gateway_key_hash:*`、`management_key_hash:*`、
  `admin_user_name:*`）保证不会退化为全量扫描；
- 用量、密钥用量和请求日志在同一 `waitUntil()` 内提交，失败不阻断模型响应；
- 密钥未配置周期预算时跳过配额 KV 读取；有限额 Key 每 isolate 默认最多 30 秒执行一次范围汇总；
- 过期统计的清理改为按需触发（`POST /admin/api/system/cleanup`），不再依赖每日 Cron；
  预算日台账至少保留 370 天；
- 熔断和 RPM 窗口保存在 isolate 内存，不产生额外 KV 写入。

边界与超额行为：KV 达到配额后配置路由可能返回 503；usage 写入或看板查询失败不会中断
