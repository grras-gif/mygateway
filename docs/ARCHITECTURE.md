...
默认只使用一个 EdgeOne Makers 项目（静态资源 + 边缘函数）和一个 KV 命名空间，加上两个
...
直接访问 KV 或任何部署 Secret。

## 3. 控制面与数据面

控制面负责管理员认证、渠道与协议端点、统一模型、Gateway Key、用量看板、Provider 余额
和系统状态。管理写操作更新 KV，并清除当前 isolate 的相关缓存；其他 isolate 通过短 TTL
收敛。
...
3. 缓存未命中时，用少量 KV `get` 完成 Key 鉴权和模型候选解析；
...
KV 是 schema-less 的键值存储。每类实体使用固定前缀，唯一性查找使用只保存 id 的二级索引键，
让鉴权与路由等热路径保持单次 `get`。
...
`src/kv/keys.ts` 是前缀与键构造的唯一权威，`src/kv/store.ts` 提供 JSON 读写与基于
`list({ prefix })` + `cursor` 的全量遍历辅助。
...
- KV 无事务，跨实体写入按“先写主实体、失败回滚已写键”的顺序补偿；
...
- 首次有效登录在 KV 创建管理员，并通过 `admin_user_name:<username>` 索引键按用户名查找；
...
KV 中 `gateway_key_hash:<hash>` 索引指向的实体比较。短 prefix 只用于控制台辨认。`expires_at` 使用
Unix 秒存入 KV；数据面在执行 RPM 与周期预算检查前判断到期时间，过期请求返回 401，控制台也会将其
...
AAD。Key 只在即将调用 Provider 时于边缘函数内解密，不进入响应、KV 明文或日志。

### Management Key

Management Key 以 `mgmt_` 开头，KV 只保存 SHA-256 hash、短前缀、`read` / `write` 权限、状态与
...
作为恢复探测。状态最多保存 500 个渠道，不写 KV 或其他共享存储，也不主动调用 Provider。
...
一次 KV 范围汇总，冷缓存并发查询会合并，并把本 isolate 已完成请求追加到本地账本。其他 isolate
...
结构化事件记录协议、模型、最终渠道、状态、耗时、尝试次数、Fallback、缓存和 KV 查询时间，
不记录 Key 或 Provider Key。请求/响应正文预览默认不保存，仅在显式开启“记录上下文”后
加密存储（4 KiB 上限、短期保留）。

- `X-Gateway-Timing`：稳定的缓存、KV 查询、访问解析、上游首包和网关首包耗时；
- `Server-Timing`：尽力提供，可能被平台指标覆盖；
- `/health`：不查询 Provider，不泄露渠道、KV 或 Secret 状态。

降级原则：usage 或看板失败不影响数据面已有响应；KV 配置读取失败时返回 503；余额失败
...
| `KEY_QUOTA_REFRESH_MS` | `30000` | 有限额 Key 每 isolate 刷新 KV 周期预算快照的间隔 |
...
的校验默认值，默认部署不在 `edgeone.json` 暴露变量表单。请求体上限可在 System 页保存到 KV，
