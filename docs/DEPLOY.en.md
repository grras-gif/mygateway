...
- Blob storage has no schema, so there is no SQL migration step. `src/db/bootstrap.ts` idempotently seeds baseline settings and model prices on the first backend request and never overwrites existing keys.
...
For production troubleshooting, inspect `X-Gateway-Timing` for cache, Blob lookup, upstream-first-byte, and gateway-first-byte timing. `Server-Timing` is also emitted. Platform logs are sampled and redacted and must not be used as exact request counts.
...
- Blob storage has no schema migration, so a new version must read existing data directly. Add new fields with backward-compatible read defaults.
...
EdgeOne Makers can roll back to a previous deployment, reverting static assets and edge functions together. Blob data does not roll back with the deployment. Prefer additive, compatible, phased data changes, and repair data with new logic instead of deleting production records.
...
Quotas change over time; verify the current [EdgeOne Makers documentation](https://edgeone.ai/document/177158575068948480) before production use. Blob read and write amplification, not request count alone, is the main capacity driver.

MyGateway stays lightweight by:

- caching bounded hot configuration in isolate memory, so cache hits never touch Blob; cold requests resolve auth and routing with a few `get`s, and unique index objects (`gateway_key_hash/*`, `management_key_hash/*`, `admin_user_name/*`) keep those lookups from degrading into full scans;
...
- keeping passive circuit breakers and RPM windows in isolate memory instead of Blob.

If Blob quota is exhausted, configuration-dependent routes may return 503. Failed analytics writes or dashboard queries do not invalidate an already completed provider response. If representative traffic approaches the project quota, reduce optional logging and analytics overhead or upgrade the project plan—do not bypass authentication or streaming correctness.
