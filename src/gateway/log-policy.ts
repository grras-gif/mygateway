 ...
 *
 * Settings live in KV and are cached per isolate for 60s so the hot path
 * does not read KV per request; admin changes propagate within a minute.
 */
...
function cachedOrDefault(db: KVNamespace, key: string): Promise<boolean> {
...
export async function readLogPolicy(db: KVNamespace): Promise<LogPolicy> {
