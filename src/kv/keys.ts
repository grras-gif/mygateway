/**
 * Blob object paths and path builders.
 *
 * Blob storage is schema-less and path-oriented: every entity type owns a fixed
 * directory prefix and every unique lookup uses a secondary index object that
 * stores only an id. This module is the single source of truth for path
 * construction; domain modules must not build prefixes inline.
 *
 * Compared with the previous KV key layout (`setting:<key>`), the logical
 * prefixes are mapped to object paths with `/` as the directory separator
 * (`setting/<key>`). Secondary index objects (`gateway_key_hash/<hash>`,
 * `management_key_hash/<hash>`, `admin_user_name/<username>`, …) stay as
 * standalone objects so hash / username reverse lookups remain a single `get`.
 */
...
/** `admin_user_name/<username>` — unique username → admin id index object. */
...
/** `gateway_key_hash/<hash>` — unique hash → Gateway Key id index object. */
...
/** `management_key_hash/<hash>` — unique hash → Management Key id index object. */
...
/** Parse a `key_usage/<key_id>/<date>` path back into its parts. */
 ...
 * bucket. The zero-padded minute keeps a prefix scan time-ordered.
 */
